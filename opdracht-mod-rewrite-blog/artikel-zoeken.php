...
<body class="web-backend-opdracht">
    ...
    <section class="body">
        ...
        $db = new mysqli('localhost', 'Indy', 'Indy', 'opdracht-mod-rewrite-blog');
                ...
                else
                {
                    ...
                    {
                        echo ("<h1>Artikels die ". '"'. $_GET['query-content'] .'"'." bevatten.</h1>");
                        $result = $db->query("SELECT * FROM artikels WHERE Artikel LIKE '%" . $_GET['query-content']. "%'");
                    }
                    if(isset($_GET['query-date']))
                    {
                        echo ("<h1>Artikels uit het jaar ". $_GET['query-date'] .".</h1>");
                        $result = $db->query("SELECT * FROM artikels WHERE Datum LIKE '%" . $_GET['query-date']. "%'");
                    }
