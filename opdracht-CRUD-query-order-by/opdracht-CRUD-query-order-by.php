<?php
	...
	try
	{
		$db = new PDO("mysql:host=".$servername.";dbname=".$dbname, $username , $password ); // Connectie maken
		...
		$query	=	'SELECT bieren.biernr,bieren.naam,brouwers.brnaam,soorten.soort,bieren.alcohol
                     FROM bieren 
				     INNER JOIN brouwers
                    ...
                    <thead>
                        <tr>
                            <?php foreach ($bierenCleanFieldnames as $key => $cleanFieldname): ?>
                                <th class="order <?= ( $order == 'ASC' && $orderColumn == $bierenFieldnames[ $key ] ) ? 'descending' : 'ascending' ?> <?= ( $orderColumn == $bierenFieldnames[ $key ] ) ? 'selected' : '' ?>">
